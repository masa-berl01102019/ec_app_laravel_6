...
<div class="container">
    <div style="display: flex; flex-flow: wrap;">
        ...
        <div class="py-5" style="width: 75%; height: 900px; display: flex; justify-content: flex-start; flex-flow: wrap; align-items: flex-start; align-content: start;">
            {{-- パンくずリスト --}}
            <div class="mb-4" style="width: 100%; height: 50px;">
                    ...
                    <ol class="breadcrumb">
                        ...
                        @if($category_id)
                            @for($i = 0; $i < count($category_id) ; $i++)
                                @if($i == 0)
                                <li class="breadcrumb-item"><a href="/category/{{$gender}}?{{ http_build_query(array_merge(Request::query(), ['gc' => $category_id[0], 'mc' => null, 'sc' => null, 'keyword' => null]), null, '&') }} ">{{ $gender }}</a></li>
                                @endif
                                @if($i == 1)
                                <li class="breadcrumb-item"><a href="/category/{{$gender}}/{{$main_category}}?{{ http_build_query(array_merge(Request::query(), ['gc' => $category_id[0], 'mc' => $category_id[1], 'sc' => null, 'keyword' => null]), null, '&') }} ">{{ $main_category }}</a></li>
                                @endif
                                @if($i == 2)
                                <li class="breadcrumb-item"><a href="/category/{{$gender}}/{{$main_category}}/{{$sub_category}}?{{ http_build_query(array_merge(Request::query(), ['gc' => $category_id[0], 'mc' => $category_id[1], 'sc' => $category_id[2], 'keyword' => null]), null, '&') }}">{{ $sub_category }}</a></li>
                                @endif
                            @endfor
                        @endif
                    </ol>
